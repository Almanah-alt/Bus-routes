...
class MyAdapter(
    ...
    inner class HintViewHolder(
        ...
        fun bindHint(schedule: Schedule){
            ...
            val btn = view.findViewById<Button>(R.id.delete_reis_btn)
            btn.setOnClickListener {
                showDialog(view, schedule)

            }
        }
    }
    fun showDialog(view:View, schedule: Schedule) {
        val dialogBuilder = AlertDialog.Builder(view.context)
        dialogBuilder.setMessage("Вы хотите удалить рейс?")
        dialogBuilder.setPositiveButton("да",
            DialogInterface.OnClickListener { dialog, whichButton ->
                AsyncTask.execute{
                    ApplicationDatabase.getInstance(context)!!
                        .getScheduleDao().deleteItem(schedule)
                }
                notifyDataSetChanged()
                Toast.makeText(view.context, "Удалён, откройте меню и заново зайдите в расписание", Toast.LENGTH_LONG).show()
            })
